...
(defn read-keys
  ...
  [c test ks]
  (->> (map (fn [k] (Thread/sleep 100) (read-key c test k)) ks)
       (zipmap ks)
       (into (sorted-map))))
    ...
    (let [c conn]
      (case (:f op)
        ...
        :inc (let [k (:value op)
               ...
               ; capture all the ordering dependencies here: an increment from
               ; x->y must fall after every read of x, and before every read of
               ; y, but the monotonic order relation can only enforce one of
               ; those. We'll return the written value here. Still better than
               ; nothing.
