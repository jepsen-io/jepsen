...
(defn final-generator
  ...
  Takes a function `(f keys) => gen` which takes a series of independent keys
  and returns a generator. Returns a generator which (when all generators are
  wrapped with `track-keys`), unfolds into that generator."
  [f]
  (FinalGenerator. f))
