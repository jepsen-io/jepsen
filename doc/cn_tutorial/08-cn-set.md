...
因为给一个集合中添加元素总是合法的。很清楚，我们需要读写结合。此外，一个读操作应该是最后发生的一个，因为最终读操作**之后**的任
何写操作将不会影响测试输出
...
我们将需要一个能往集合中添加元素，并能读取元素的一个client--但我们必须选择如何在数据库中存储上面的集合set。一个选项是使用独立的key，或者一个key池子。另一个选项是使用单个key，并且其value是一个序列化的数据类型，类似于json数组或者Clojure的set，我们将使用后者。
...
多考虑一下generator...我们知道它将处理两个阶段：添加和最终读取。我们也知道我们想要读取成功，这意味着我们想让集群正常并且恢复那一点，
