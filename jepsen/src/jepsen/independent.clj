...
(defn track-keys
  "Wraps a generator producing operations with independent tuples. Keeps track
  of the set of all keys observed, and makes that set available in
  (:jepsen.independent/keys context) to the wrapped generator."
  [gen]
  (gen/track ::keys
             #{}
             (fn [ks op]
...
(defrecord FinalGenerator [f]
  gen/Generator
  (op [this test ctx]
    (gen/op (map (fn key-gen [k]
                   (gen/map (fn lift [op]
                              (assoc op :value (tuple k (:value op))))
                            (f k)))
                 (::keys ctx))
...
(defn final-generator
  "Use this to perform some final operation at the end of a test for every key
  generated by an independent generator.

  Takes a function `(f k) => gen` which takes a key and returns a generator for
  that key. Returns a generator which (when all generators are wrapped with
  `track-keys`), unfolds into that generator, and with all values lifted to [k
  original-value]. For example, to perform one read of every key, you could use
  `(final-generator (constantly {:f :read}))`, which will emit operations like
  {:f :read, :value [2 nil]`, where `2` is one of the keys."
  [f]
  (FinalGenerator. f))
