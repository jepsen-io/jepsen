...
(deftest independent-concurrent-test
  ...
  (is (= [[0 4 [:k2 :v0]]
          ...
          [10 1 [:k0 :v2]]
          ; Worker 0 in group 0 starts k3
          [10 0 [:k3 :v0]]
          ; Worker 5 in group 2 finishes k2
          [10 5 [:k2 :v2]]
          ; Worker 3 in group 1 finishes k1
          [10 3 [:k1 :v2]]
          ; And worker 2 in group 1 starts k4
          [10 2 [:k4 :v0]]

          ; All keys have now started execution. Group 2 (workers 4 and 5) has
          ; process 4 free, but can't start a new key since there are none left
          ; pending.
          ; Workers 2 and 3 in group 1 can finish k4
          [20 2 [:k4 :v1]]
          [20 3 [:k4 :v2]]
          ; Worker 0 in group 0 can continue k3
          [20 0 [:k3 :v1]]
          ; Finally, worker 1 in group 0 finishes k3
          [20 1 [:k3 :v2]]]
         (->> (independent/concurrent-generator
                ...
                (fn [k]
                  (->> [:v0 :v1 :v2] ; Three values per key
                       (map (partial hash-map :type :invoke, :value)))))
              ; 3 groups of 2 threads each
              (gen.test/perfect (gen.test/n+nemesis-context 6))
              (map (juxt :time :process :value))))))
